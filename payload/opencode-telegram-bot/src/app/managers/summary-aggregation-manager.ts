...
function prepareImageAttachment(attachment: {
  mime: string;
  url: string;
  filename?: string;
}): CodeFileData | null {
  if (!attachment.mime.toLowerCase().startsWith("image/") || !attachment.url.startsWith("data:")) {
    return null;
  }
  ...
  const metadata = attachment.url.slice("data:".length, commaIndex);
  const parameters = metadata.split(";").map((parameter) => parameter.trim().toLowerCase());
  if (!parameters.includes("base64")) {
    return null;
  }

  const mime = parameters[0] || attachment.mime;
  const encoded = attachment.url.slice(commaIndex + 1).replace(/\s/g, "");
  if (!encoded || !/^[A-Za-z0-9+/]*={0,2}$/.test(encoded)) {
    return null;
  }

  const buffer = Buffer.from(encoded, "base64");
  if (buffer.length === 0 || buffer.length > MAX_IMAGE_ATTACHMENT_BYTES) {
    return null;
  }

  const filename = (attachment.filename || "generated-image.jpg")
      ...
      const attachment = Array.isArray(attachments)
        ? attachments.find(
            (value): value is { mime: string; url: string; filename?: string } =>
              isRecord(value) && typeof value.mime === "string" && typeof value.url === "string",
          )
        : undefined;
      const fileData = attachment ? prepareImageAttachment(attachment) : null;
      return { fileData, fileChange: null };
    }
